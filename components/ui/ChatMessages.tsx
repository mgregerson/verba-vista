  ...
  return (
    <div className="p-5">
      {!loading && messages?.length === 0 && (
        <div className="flex flex-col justify-center items-center p-20 rounded-xl space-y-2 bg-indigo-400 text-white font-extralight h-[100px] text-center">
      ...
      {messages?.map((message) => {
        ...
        return (
          <div key={message.id} className="flex my-2 items-end">
            <div
              className={`flex flex-col relative space-y-2 p-3 w-fit line-clamp-1 mx-2 roundedlg ${
                isSender
                  ? "ml-auto bg-violet-600 textwhite rounded-md"
                  : "bg-gray-100 dark:text-gray-100 dark:bg-slate-700 rounded-md"
              }`}
            >
              <p
